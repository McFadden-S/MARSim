...
class PygameRenderer:
    ...
    Usage::
        ...
        renderer = PygameRenderer(width=50, height=50, obs_radius=5)
    ...
    def draw_static(self, obstacles):
        ...
        self.frame_hi = pygame.Surface((self._hi_w, self._hi_h), flags=pygame.SRCALPHA).convert_alpha()
    ...
    def _draw_grid_hi(self):
        ...
        for j in range(self.width + 1):
            ...
            pygame.draw.line(self.static_hi, self._COL_GRID,
                             (x, self.pad_hi), (x, self.pad_hi + self.height * self.cell_px_hi), width=sw)
        for i in range(self.height + 1):
            y = self.pad_hi + i * self.cell_px_hi
            pygame.draw.line(self.static_hi, self._COL_GRID,
                             (self.pad_hi, y), (self.pad_hi + self.width * self.cell_px_hi, y), width=sw)
    ...
    def draw_state(self, agents_xy, targets_xy, agent_types, is_active):
        """Render all active agents and their target waypoints for one frame."""
        ...
        for idx, (ai, aj) in enumerate(agents_xy):
            ...
            if atype in (AgentType.FRIENDLY_UAV, AgentType.ENEMY_UAV):
                ...
                pygame.draw.circle(self.frame_hi, colour, (ctx, cty), rpx_hi, width=rpx_hi // 4)

            elif atype == AgentType.FRIENDLY_UGV:
                self._draw_ugv(cx, cy, rpx_hi)
                pygame.draw.circle(self.frame_hi, self._COL_FRI, (ctx, cty), rpx_hi, width=rpx_hi // 4)

            else:
                pygame.draw.circle(self.frame_hi, self._COL_UNK, (cx, cy), rpx_hi)
